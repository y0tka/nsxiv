...
bool cg_first(arg_t _) { return navigate_to(0); }
...
bool cg_change_gamma(arg_t d) { return change_color_modifier(d, &img.gamma); }
...
bool ci_cursor_navigate(arg_t _) { return ci_navigate(nav_button() - 1); }
...
bool ci_scroll(arg_t dir) { return img_pan(&img, dir, prefix); }

bool ci_scroll_to_center(arg_t _) { return img_pan_center(&img); }

bool ci_scroll_to_edge(arg_t dir) { return img_pan_edge(&img, dir); }
...
bool ci_drag(arg_t drag_mode) {
  ...
  if ((int)(img.w * img.zoom) <= (int)win.w &&
      (int)(img.h * img.zoom) <= (int)win.h)
    return false;

  win_set_cursor(&win, drag_mode == DRAG_ABSOLUTE ? CURSOR_DRAG_ABSOLUTE
...
bool ci_fit_to_win(arg_t sm) { return img_fit_win(&img, sm); }
...
bool ct_scroll(arg_t dir) { return tns_scroll(&tns, dir, false); }
