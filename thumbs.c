...
void tns_init(tns_t *tns, fileinfo_t *tns_files, const int *cnt, int *sel,
              win_t *win) {
  int len;
...
static Imlib_Image tns_scale_down(Imlib_Image im, int dim) {
  ...
  if (z < 1.0) {
    ...
    im = imlib_create_cropped_scaled_image(0, 0, w, h, MAX(z * w, 1),
                                           MAX(z * h, 1));
    if (im == NULL)
...
void tns_highlight(tns_t *tns, int n, bool hl) {
  if (n >= 0 && n < *tns->cnt && tns->thumbs[n].im != NULL) {
    ...
    win_draw_rect(win, t->x - oxy, t->y - oxy, t->w + owh, t->h + owh, false,
                  tns->bw, col);
