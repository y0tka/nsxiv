...
static struct {
  int name;
  Cursor icon;
} cursors[CURSOR_COUNT] = {{XC_left_ptr},      {XC_dotbox},
                           {XC_fleur},         {XC_watch},
...
static void xft_alloc_color(const win_env_t *e, const char *name,
                            XftColor *col) {
...
static const char *win_res(XrmDatabase db, const char *name, const char *def) {
  ...
  if (db != NULL && XrmGetResource(db, name, name, &type, &ret) &&
...
void win_open(win_t *win) {
  ...
  win->xwin =
      XCreateWindow(e->dpy, parent, win->x, win->y, win->w, win->h, 0, e->depth,
                    InputOutput, e->vis, CWColormap | CWBorderPixel, &attrs);
  if (win->xwin == None)
    error(EXIT_FAILURE, 0, "Error creating X window");

  /* set the _NET_WM_PID */
  pid = getpid();
  XChangeProperty(e->dpy, win->xwin, atoms[ATOM__NET_WM_PID], XA_CARDINAL, 32,
  ...
  *cnone = XCreatePixmapCursor(e->dpy, none, none, &col, &col, 0, 0);
  ...
  if (options->fullscreen) {
    XChangeProperty(e->dpy, win->xwin, atoms[ATOM__NET_WM_STATE], XA_ATOM, 32,
                    PropModeReplace,
...
bool win_configure(win_t *win, XConfigureEvent *c) {
  ...
  changed = win->w != (unsigned int)c->width ||
            win->h + win->bar.h != (unsigned int)c->height;
...
#if HAVE_LIBFONTS
static int win_draw_text(win_t *win, XftDraw *d, const XftColor *color, int x,
                         int y, char *text, int len, int w) {
...
static void win_draw_bar(win_t *win) {
  ...
  XFillRectangle(e->dpy, win->buf.pm, gc, 0, win->bar.top ? 0 : win->h, win->w,
                 win->bar.h);
...
#else
static void win_draw_bar(win_t *win) { (void)win; }
#endif /* HAVE_LIBFONTS */
...
void win_set_title(win_t *win, const char *title, size_t len) {
  int i, targets[] = {ATOM_WM_NAME, ATOM_WM_ICON_NAME, ATOM__NET_WM_NAME,
