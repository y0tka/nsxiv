...
void img_init(img_t *img, win_t *win) {
  ...
  img->multi.framedelay =
      options->framerate > 0 ? 1000 / options->framerate : 0;
  img->multi.length = 0;
  ...
  img->ss.delay =
      options->slideshow > 0 ? options->slideshow : SLIDESHOW_DELAY * 10u;
}
      ...
      for (i = 0; i < sh; i++) {
        for (j = 0; j < sw; j++) {
          if (i < y || i >= y + h || j < x || j >= x + w ||
              rows[i - y][j - x] == transp) {
            if (prev_frame != NULL &&
                (prev_disposal != 2 || i < py || i >= py + ph || j < px ||
                 j >= px + pw)) {
...
static bool img_load_webp(img_t *img, const fileinfo_t *file) {
  ...
  if ((err = (dec == NULL) || !WebPAnimDecoderGetInfo(dec, &info))) {
    error(0, 0, "%s: WebP parsing or memory error (file is corrupt?)",
          file->name);
    goto fail;
  }
...
static bool img_load_multiframe(img_t *img, const fileinfo_t *file) {
  ...
  if ((fcnt = finfo.frame_count) <= 1 ||
      !(finfo.frame_flags & IMLIB_IMAGE_ANIMATED))
    return false;
  ...
  for (n = 1; n <= fcnt; ++n) {
    ...
    imlib_blend_image_onto_image(frame, has_alpha, 0, 0, sw, sh, sx, sy, sw,
                                 sh);
    m->frames[m->cnt].im = canvas;
    m->frames[m->cnt].delay =
...
Imlib_Image img_open(const fileinfo_t *file) {
  ...
  if (access(file->path, R_OK) == 0 && stat(file->path, &st) == 0 &&
      S_ISREG(st.st_mode) &&
#if HAVE_IMLIB2_MULTI_FRAME
 ...
 */
  ...
  if ((fmt = imlib_image_format()) !=
      NULL) { /* NOLINT: fmt might be unused, not worth fixing */
#if HAVE_LIBGIF && !HAVE_IMLIB2_MULTI_FRAME
    if (STREQ(fmt, "gif"))
...
bool img_zoom_to(img_t *img, float z) {
  ...
  if (ZOOM_MIN <= z && z <= ZOOM_MAX) {
    ...
    if (x < 0 || (unsigned int)x >= img->win->w || y < 0 ||
        (unsigned int)y >= img->win->h) {
      x = img->win->w / 2;
      y = img->win->h / 2;
    }
...
bool img_zoom(img_t *img, int d) {
  ...
  while (i >= 0 && i < (int)ARRLEN(zoom_levels) &&
         (d > 0 ? zoom_levels[i] / 100 <= img->zoom
                : zoom_levels[i] / 100 >= img->zoom)) {
    i += d;
  }
...
void img_flip(img_t *img, flipdir_t d) {
  unsigned int i;
  void (*imlib_flip_op[3])(void) = {imlib_image_flip_horizontal,
                                    imlib_image_flip_vertical,
...
void img_update_color_modifiers(img_t *img) {
  ...
  if (img->gamma != 0)
    imlib_modify_color_modifier_gamma(
        steps_to_range(img->gamma, GAMMA_MAX, 1.0));
  if (img->brightness != 0)
    imlib_modify_color_modifier_brightness(
        steps_to_range(img->brightness, BRIGHTNESS_MAX, 0.0));
  if (img->contrast != 0)
    imlib_modify_color_modifier_contrast(
        steps_to_range(img->contrast, CONTRAST_MAX, 1.0));
...
static bool img_frame_goto(img_t *img, int n) {
  if (n < 0 || (unsigned int)n >= img->multi.cnt ||
      (unsigned int)n == img->multi.sel)
    return false;
