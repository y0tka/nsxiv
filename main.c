...
static struct {
  ...
  bool active;
    ...
    {redraw}, {reset_cursor}, {slideshow}, {animate}, {clear_resize},
};
...
static void close_title(void) { kill_close(wintitle.pid, &wintitle.fd); }
...
static void open_title(void) {
  ...
  construct_argv(argv, ARRLEN(argv), wintitle.f.cmd, files[fileidx].path, fidx,
                 fcnt, w, h, z, NULL);
...
void close_info(void) { kill_close(info.pid, &info.fd); }
...
void clear_resize(void) { resized = false; }
...
static bool run_key_handler(const char *key, unsigned int mask) {
  ...
  snprintf(kstr, sizeof(kstr), "%s%s%s%s", mask & ControlMask ? "C-" : "",
           mask & Mod1Mask ? "M-" : "", mask & ShiftMask ? "S-" : "", key);
...
static bool process_bindings(const keymap_t *bindings, unsigned int len,
...
static void on_buttonpress(const XButtonEvent *bev) {
  ...
  if (mode == MODE_IMAGE) {
    ...
    reset_cursor();
    dirty = process_bindings(buttons_img, ARRLEN(buttons_img), bev->button,
                             bev->state, 0);
  } else { /* thumbnail mode */
    dirty = process_bindings(buttons_tns, ARRLEN(buttons_tns), bev->button,
                             bev->state, 0);
  }
  if (dirty)
    redraw();
  ...
  while (true) {
    ...
    if ((init_thumb || load_thumb || to_set || info.fd != -1 || arl.fd != -1) &&
      ...
      if (load_thumb) {
        ...
        pfd[FD_X].events = pfd[FD_INFO].events = pfd[FD_TITLE].events =
            pfd[FD_ARL].events = POLLIN;
...
int main(int argc, char *argv[]) {
  ...
  if (homedir != NULL) {
    ...
    const char *name[] = {"image-info", "thumb-info", "key-handler",
                          "win-title"};
